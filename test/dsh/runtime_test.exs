defmodule TeardownProbeConsumer do
  ...
  def handle_dsh_withdraw(_keys, state) do
    ...
    result = Dsh.Session.count(state.view.session)
...
defmodule Dsh.RuntimeTest do
  use ExUnit.Case, async: false

  defp session_entry, do: %{id: :session, plugin: Dsh.Session.Plugin, config: [], disabled: false}
  ...
  test "boots an ordered composition; the consumer resolves and uses the session" do
    {:ok, runtime} = Dsh.Runtime.start_link([session_entry(), consumer_entry()], [])
    ...
    assert {:ok, 2} = Dsh.Session.append(session, %{"role" => "assistant", "text" => "hello"})

    assert Dsh.Session.all(session) == [
             %{"role" => "user", "text" => "hi"},
             %{"role" => "assistant", "text" => "hello"}
           ]

    assert Dsh.Consumer.view(consumer) == %{session: session}
    assert Dsh.Runtime.entries(runtime) |> Map.keys() == [:session, :consumer]
  end
  ...
  test "removing the provider deactivates the dependent before withdrawal" do
    ...
    :ok = Dsh.Runtime.reconcile(runtime, [consumer_entry()])
  ...
  test "a provider swap reactivates only the dependent" do
    ...
    :ok =
      Dsh.Runtime.reconcile(runtime, [consumer_entry(), sibling_entry, sibling_consumer_entry])

    assert_receive {:consumer_deactivated, :consumer, ^consumer, [:session], _view}, 2000
    assert Dsh.Consumer.state(consumer) == :inactive
    assert Dsh.Consumer.state(sibling) == :active
    ...
    new_session_entry = %{
      id: :session,
      plugin: Dsh.Session.Plugin,
      config: [provider: Dsh.Session.File, path: path],
      disabled: false
    }

    :ok =
      Dsh.Runtime.reconcile(runtime, [
        new_session_entry,
        consumer_entry(),
        sibling_entry,
        sibling_consumer_entry
    ...
    assert {:ok, session} = Dsh.Context.get(ctx, :session)
    assert Dsh.Session.count(session) == 0
  end
  ...
  test "a dependent still reads its committed view during its own teardown" do
    ...
    {:ok, runtime} = Dsh.Runtime.start_link([session_entry(), probe_entry], [])

    assert_receive {:probe_registered, probe, :active, %{session: session}}, 2000
    assert {:ok, 1} = Dsh.Session.append(session, "one")
    assert {:ok, 2} = Dsh.Session.append(session, "two")

    :ok = Dsh.Runtime.reconcile(runtime, [probe_entry])
  ...
  test "a crashed provider's resource survives until dependents drained (ordered shutdown)" do
    ...
    {:ok, plugin} = Dsh.Session.Plugin.start_link(ctx, id: :session)
  ...
  test "a start failure is reported instead of swallowed (A-H2 regression)" do
    ...
    assert {:error, [{:bad, _reason}]} = Dsh.Runtime.reconcile(runtime, [entry])
    assert %{bad: %{pid: nil}} = Dsh.Runtime.entries(runtime)
  end
  ...
  test "a crashing plugin is re-injected up to the restart limit, then given up" do
    ...
    :ok = Dsh.Runtime.reconcile(runtime, [entry])

    wait_until(fn -> match?(%{loop: %{pid: nil}}, Dsh.Runtime.entries(runtime)) end)

    assert %{loop: %{error: :crash_loop}} = Dsh.Runtime.entries(runtime)
  end

  test "the quiescent state is a function of the final configuration (confluence)" do
    entry_a = %{id: :a, plugin: Dsh.Provider, config: [provides: %{alpha: 1}], disabled: false}
    entry_b = %{id: :b, plugin: Dsh.Provider, config: [provides: %{beta: 2}], disabled: false}

    # path 1: a, then a+b
    {:ok, r1} = Dsh.Runtime.start_link([], [])
    :ok = Dsh.Runtime.reconcile(r1, [entry_a])
    :ok = Dsh.Runtime.reconcile(r1, [entry_a, entry_b])

    # path 2: b, then a+b
    {:ok, r2} = Dsh.Runtime.start_link([], [])
    :ok = Dsh.Runtime.reconcile(r2, [entry_b])
    :ok = Dsh.Runtime.reconcile(r2, [entry_a, entry_b])

    # from scratch
    {:ok, r3} = Dsh.Runtime.start_link([entry_a, entry_b], [])

    # a canonical state: binding keys and fiber id/state summaries (pids differ)
    canonical = fn runtime ->
      %{bindings: bindings, fibers: fibers} = Dsh.Context.snapshot(Dsh.Runtime.context(runtime))
  ...
  test "concurrent reconciles to one configuration converge without corruption" do
    ...
    entry = %{id: :x, plugin: Dsh.Provider, config: [provides: %{omega: 1}], disabled: false}

    tasks =
      for _ <- 1..10 do
        Task.async(fn -> Dsh.Runtime.reconcile(runtime, [entry]) end)
      end

    assert Enum.all?(Task.await_many(tasks, 5000), &(&1 == :ok))

    ctx = Dsh.Runtime.context(runtime)
    assert Dsh.Context.get(ctx, :omega) == {:ok, 1}
  end
  ...
  test "concurrent use during a provider swap converges to the new provider" do
    ...
    writer =
      Task.async(fn ->
        for i <- 1..200 do
          case Dsh.Context.get(ctx, :session) do
            {:ok, session} when is_pid(session) ->
              try do
                Dsh.Session.append(session, i)
              catch
    ...
    :ok = Dsh.Runtime.reconcile(runtime, [consumer_entry()])
    :ok = Dsh.Runtime.reconcile(runtime, [new_session_entry, consumer_entry()])

    assert_receive {:consumer_activated, :consumer, ^consumer, [:session]}, 2000
    assert Task.await(writer, 5000) == :done

    wait_until(fn ->
      match?({:ok, session} when is_pid(session), Dsh.Context.get(ctx, :session))
    end)

    assert {:ok, new_session} = Dsh.Context.get(ctx, :session)
    assert Dsh.Session.count(new_session) >= 0
    assert Dsh.Consumer.view(consumer) == %{session: new_session}
  end
