...
defmodule CrashOnDemand do
  ...
  def init({ctx, _opts}) do
    {:ok, _state, _view} =
      DshBeam.Context.register(ctx, id: :boom, deps: [], provides: %{boom: :value})
...
defmodule DshBeam.FiberTest do
  ...
  test "fiber lifecycle states are observable through the context mirror" do
    ...
    wait_until(fn -> DshBeam.Context.fiber_state(ctx, consumer) == :active end)

    :ok = DshBeam.Context.unload(ctx, provider)
    wait_until(fn -> DshBeam.Plugin.fiber_state(consumer) == :inactive end)
    wait_until(fn -> DshBeam.Context.fiber_state(ctx, consumer) == :inactive end)
  end
  ...
  test "the guard holds while a dependent is mid-teardown (:unloading)" do
    ...
    assert DshBeam.Plugin.fiber_state(slow) == :inactive
  end

  test "a fiber crashing after registration is isolated; its bindings are recovered" do
    {:ok, ctx} = DshBeam.Context.start_link([])
    {:ok, _sibling} = DshBeam.Provider.start_link(ctx, id: :sibling, provides: %{other: :ok})
    {:ok, fiber} = CrashOnDemand.start_link(ctx, [])

    # registration completed: the binding is visible
    wait_until(fn -> DshBeam.Context.get(ctx, :boom) == {:ok, :value} end)
    ...
    wait_until(fn -> DshBeam.Context.get(ctx, :boom) == :not_found end)

    assert DshBeam.Context.get(ctx, :boom) == :not_found
    assert DshBeam.Context.get(ctx, :other) == {:ok, :ok}
  end
